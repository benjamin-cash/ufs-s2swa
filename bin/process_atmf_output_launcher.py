...
def main():
    ...
    for count,netcdf_index in enumerate(netcdf_indices):
        for vname in vlist:
            print(os.environ['CYLC_TASK_PARAM_mem'], os.environ['CYLC_TASK_PARAM_ldate'], netcdf_index, out_indices[count],
                vname)
            netcdf_fname =  os.path.join(os.environ['RUNDIR'],"atmf"+netcdf_index+".nc")
            y = utils.get_subset(netcdf_fname, vname)
            ds = xr.open_dataset(netcdf_fname)
            y.attrs['ak'] = ds.attrs['ak']
            y.attrs['bk'] = ds.attrs['bk']
            utils.write_compressed_file(y, os.environ['POSTDIR'], os.environ['CYLC_TASK_PARAM_ldate'],
                os.environ['CYLC_TASK_PARAM_mem'], vname, "atmf", out_indices[count])
